...
from wesep.models.local.PositionalEncoding import PositionalEncoding
from wesep.models.local.TFgridnet import GridNetV2Block

from wesep.utils.feature import STFT, iSTFT
from wesep.models.local.TFgridnet import TF_gridnet_attentionblock

EPS = 1e-8


class USEF_TFGridNet(nn.Module):
    ...
    def __init__(
        ...
        super(USEF_TFGridNet, self).__init__()
        ...
        self.att = TF_gridnet_attentionblock(emb_dim=emb_dim, n_freqs=n_freqs, n_head=n_head, approx_qk_dim=approx_qk_dim)
    ...
    def forward(self, input, aux):
        ...
        aux_ri = torch.cat([aux_c.real, aux_c.imag],dim = 1)    # B, 2, F, T,
        aux_ri = aux_ri.permute(0,1,3,2).contiguous()

        mix_ri = self.conv(mix_ri)      # B, 128, F, T,
        aux_ri = self.conv(aux_ri)

        aux_ri = self.att(mix_ri, aux_ri)   # B, 128, F, T,
