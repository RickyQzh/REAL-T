...
class CAMPPlus(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.head(x)
        x_frame = self.xvector(x)

        x = self.classifier(x_frame)
        return x_frame, x
...
if __name__ == '__main__':
    ...
    # pretrained_model = torch.load('/mntcephfs/lab_data/wangxuefei/zk/wesep/examples/librimix/tse/v1/wespeaker_campplus_damo200k/avg_model.pt')
    ...
    #         old_key = key.replace('classifier', 'xvector')
    #         state[key] = pretrained_model[old_key]
    #     else:
    #         print("not %s loaded" % key)
    # model.load_state_dict(state)
    # torch.save(model.state_dict(), '/mntcephfs/lab_data/wangxuefei/zk/wesep/examples/librimix/tse/v1/wespeaker_campplus_damo200k/avg_model_wesep.pt')
