...
class CrossFuseLayer(nn.Module):
    def __init__(self, embed_dim=256, feat_dim=512, fuse_type='cross_concat'):
        super(CrossFuseLayer, self).__init__()
        assert fuse_type in ["cross_concat", "cross_additive", "cross_multiply", "cross_FiLM", "None"]

        self.fuse_type = fuse_type.split('_')[-1]
        if self.fuse_type == 'concat':
            self.fc = nn.Linear(embed_dim + feat_dim, feat_dim)
        elif self.fuse_type == 'additive':
            self.fc = nn.Linear(embed_dim, feat_dim)
    ...
    def forward(self, x, embed):
        ...
        if self.fuse_type == "concat":
            ...
            if len(x.size()) == 3:
                # embed_t = embed.expand(-1, -1, x.size(2))
                y = torch.cat([x, embed], 1)
                y = torch.transpose(y, 1, 2)
                x = torch.transpose(self.fc(y), 1, 2)
            else:
                # len(x.size() == 4
                # embed_t = embed.expand(-1, x.size(1), -1, x.size(3))
                y = torch.cat([x, embed], 2)
                y = torch.transpose(y, 2, 3)
                x = torch.transpose(self.fc(y), 2, 3).contiguous()
                # print(x.size())
        elif self.fuse_type == "additive":
            if len(x.size()) == 3:
                # embed_t = embed.expand(-1, -1, x.size(2))
                embed_t = torch.transpose(embed, 1, 2)
                x = x + torch.transpose(self.fc(embed_t), 1, 2)
            else:
                # len(x.size() == 4
                # embed_t = embed.expand(-1, x.size(1), -1, x.size(3))
                embed_t = torch.transpose(embed, 2, 3)
                x = x + torch.transpose(self.fc(embed_t), 2, 3)
        elif self.fuse_type == "multiply":
            if len(x.size()) == 3:
                # embed_t = embed.expand(-1, -1, x.size(2))
