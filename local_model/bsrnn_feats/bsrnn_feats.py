...
from speaker import PreEmphasis
from speaker import SpeakerFuseLayer
from speaker import SpeakerTransform
from funcs import compute_fbank, apply_cmvn
from resnet import ModelWrapper as Aslp_Speaker
...
class BSRNN_Feats_Local(nn.Module):
    ...
    def __init__(
        ...
        super(BSRNN_Feats_Local, self).__init__()
        ...
        if joint_training and (spk_fuse_type or spectral_feat == 'tfmap_emb'):
            ...
            # modify,using resnet
            print("speaker_net")
            if speaker_net == "resnet":
                
           
                self.speaker_net = Aslp_Speaker()
                
                if spk_model_init:
            
                    
                    cpt = torch.load(spk_model_init, map_location="cpu")
                    for i in list(cpt):
                        if i=='projection.weight':
                            del cpt[i]
                    self.speaker_net.embedding_model.load_state_dict(cpt)

                    print("successful to reload_spk: ", spk_model_init)
            elif speaker_net == "ECAPA_TDNN_GLOB_c512":
                 self.speaker_net = get_speaker_model(speaker_net)(**spk_args)
                 if spk_model_init:
                    pretrained_model = torch.load(spk_model_init)
                    print("load pretrain spk model")
                    state = self.speaker_net.state_dict()
                    ...
                    self.speaker_net.load_state_dict(state)
                    print("successful to reload_spk: ", spk_model_init)
    ...
    def forward(self, input, embeddings):
        ...
        if self.spectral_feat:
            ...
            if self.spectral_feat == 'tfmap_emb':  # Only Ecapa-TDNN.
                ...
                spk_emb = self.speaker_net(spk_emb)
                ...
                mix_emb = self.speaker_net(mix_emb)
                ...
                assert not torch.isnan(spk_emb_frame_).any(), "spk_emb"
                assert not torch.isnan(mix_emb_frame_).any(), "mix_emb"

                mix_emb_frame_ = mix_emb_frame_.transpose(1, 2)
                att_scores = torch.matmul(mix_emb_frame_, spk_emb_frame_)
                assert not torch.isnan(att_scores).any(), "att_scores"
                att_weights = F.softmax(att_scores, dim=-1)
                ...
                # enroll_mag = F.normalize(enroll_mag, p=2, dim=1)
                tf_map = torch.matmul(att_weights, enroll_mag)
                assert not torch.isnan(tf_map).any(), "tf_map"
                tf_map = tf_map.transpose(1, 2)

                tf_map = tf_map / tf_map.norm(dim=1, keepdim=True)
                assert not torch.isnan(tf_map).any(), "tf_map-2"
                # Recover the energy of estimated tfmap feature
                tf_map = (
                    torch.sum(mix_mag_ori * tf_map, dim=1, keepdim=True) 
                    * tf_map
                )
                assert not torch.isnan(tf_map).any(), "tf_map-3"
                # Another kind of nomalization for tf_map feature
                # tf_map = tf_map * mix_mag_ori.norm(dim=1, keepdim=True)

                spec_RI = torch.cat((spec_RI, tf_map.unsqueeze(1)), dim=1)
            if self.spectral_feat == 'tfmap_emb_resnet':  # Only Ecapa-TDNN.
                with torch.no_grad():
                    signal_dim = wav_input.dim()
                    extended_shape = (
                        [1] * (3 - signal_dim) 
                ...
                assert not torch.isnan(spk_emb).any(), "spk_emb"
                if isinstance(spk_emb, tuple):
                    spk_emb_frame = spk_emb[0]
                else:
                    spk_emb_frame = spk_emb
        ...
        if (
            ...
            if self.spk_fuse_type and self.spk_fuse_type.startswith("cross_"):
                tmp_spk_emb_input = self.speaker_net._get_frame_level_feat(
                    spk_emb_input)
            else:
                tmp_spk_emb_input = self.speaker_net(spk_emb_input)
...
if __name__ == "__main__":
    ...
    model = BSRNN_Feats_Local(
        ...
        # 添加
        spk_model_init="wespeaker-voxceleb-ecapa-tdnn512.pt",
        spk_model_freeze=True,
        
        spectral_feat='tfmap_emb',
        spk_fuse_type='cross_multiply',
        speaker_net="ECAPA_TDNN_GLOB_c512",
        spk_args={
            "embed_dim": 192,
            "feat_dim": 80,
            "pooling_func": "ASTP",
    ...
    spk_embeddings = torch.randn(1, 16000)
    output = model(x, spk_embeddings)
    print(output['wav'].shape)

    macs, params = profile(model, inputs=(x, spk_embeddings))
    macs, params = clever_format([macs, params], "%.3f")
    print(macs, params)
"""
# of parameters: 26.488046646118164
load pretrain spk model
successful to reload_spk:  wespeaker-voxceleb-ecapa-tdnn512.pt
# of parameters: 26.488046646118164
torch.Size([1, 16000])
...
'''
修改源码，适配batch=1
/home/work_nfs11/zkliu/anaconda3/envs/urgent26/lib/python3.10/site-packages/wespeaker/models/ecapa_tdnn.py
'''